...
## Creating a 3D Slice
To view 4D objects, we can take a 3D slice of its geometry. To create a slice of a given `OFD_Mesh`, we can call `OFD_SliceMesh`, providing the mesh we'd like to take a slice from, and the W position of our slice. This function will return a pointer to an array of `OFD_Triangle`s. 

As well as providing an array of `OFD_Triangle`s, Open4D also provides a series of functions to gather vertex positions for use outside of Open4D.
